...
- Panes and windows to organize code, R console, help, etc.

## Alternatives

- RStudio (by posit, formerly RStudio PBC)\
...
`📦 remotes` for most other sources
...
Valid names in R may only include letters, numbers, dashes, and underscores. Names may not start with a number
...
**camelCase** - after fist word identify different words with uppercase, no spaces

**snake_case** - all lowercase, underscores separate words

**slug-case** - all lowercase, dashes separate words  

---

</br></br>
**Title Case** - Capitalize words except articles, use spaces

**Sentence case** - Capitalize first word, use spaces
</br>
</br>

We can transform text strings between different letter cases using the 📦 `snakecase` package
...
my_table <- read.csv(file = "my_folder/my-file.csv",
                     header = TRUE) 
